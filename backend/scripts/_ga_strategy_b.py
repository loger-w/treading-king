"""策略 B（mountain_drift_break）GA 參數優化。

染色體 = [drift_bars, drift_ratio, break_confirm_bars, require_below_vwap, levels_combo, tolerance_pct]
造山參數固定用 v4 defaults。
適應度 = 獲利訊號數 × abs(平均跌幅) − α × 虧損訊號數

用法:
  .venv\\Scripts\\python scripts\\_ga_strategy_b.py --day 2026-06-16
  .venv\\Scripts\\python scripts\\_ga_strategy_b.py --day 2026-06-16 --pop 60 --gen 120
"""
import argparse
import json
import math
...
GENES = [
    ("drift_bars",         3, 15, 1),
    ("drift_ratio",        0.4, 0.9, 0.1),
    ("break_confirm_bars", 1, 4, 1),
...
DEFAULTS = (8, 0.6, 2, 0, 6, 0.0)  # drift=8, ratio=0.6, confirm=2, no vwap, all 3, no tol
...
def preprocess(cands, pool, minute_data, cdp_data, day):
    ...
    for sym in cands:
        ...
        if not ci:
            continue
        cdp_levels = {"nl": ci.get("nl", 0), "al": ci.get("al", 0), "cdp": ci.get("cdp", 0)}
...
def eval_chromosome(params, stocks_list, alpha):
    drift_bars, drift_ratio, break_confirm, require_vwap, levels_idx, tolerance_pct = params
    drift_bars = int(drift_bars)
    break_confirm = int(break_confirm)
        ...
        for _sym, candles, cdp_levels, day_close, cum, vols in stocks:
            ...
            for i in range(n):
                ...
                if prev_close is not None:
                    drift_window.append(c < prev_close)
                    if len(drift_window) > drift_bars:
                        del drift_window[:-drift_bars]
                prev_close = c

                if len(drift_window) < drift_bars:
                    continue

                drift_down = sum(drift_window)
                drift_ok = drift_down >= drift_threshold
                ...
                for lname in levels:
                    ...
                    if c < threshold:
                        cnt = break_counts.get(lname, 0) + 1
                        break_counts[lname] = cnt
                        if cnt >= break_confirm:
...
def ga_run(stocks_list, alpha, pop_size=60, n_gen=120, elite_n=6, seed=42):
    random.seed(seed)
...
def fmt_params(p):
    db, dr, bc, vwap, li, tol = p
    lvs = "+".join(l.upper() for l in LEVELS_COMBOS[int(li)])
    return (f"drift={int(db)} ratio={dr:.1f} confirm={int(bc)} "
            f"vwap={'Y' if vwap else 'N'} levels={lvs} tol={tol:.1f}%")
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print(f"  策略 B (mountain_drift_break) GA 優化")
    ...
    out_path = CACHE_DIR / f"_ga_strategy_b_result_{'_'.join(days)}.json"
