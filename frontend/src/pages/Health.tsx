...
function SystemStatus() {
  ...
  return (
    <section>
      ...
      <h2 className="h-display text-[36px] mb-6">Health</h2>

      <p className="font-serif text-[19px] leading-[1.5] text-ink mb-6">
        The backend reports two dependencies. <em className="font-serif italic text-accent">Fubon</em>{" "}
        — the upstream market-data provider. <em className="font-serif italic text-accent">Supabase</em>{" "}
        — the persistent store for symbols, strategies, and signal history.
      </p>

      {error && (
        <div className="border border-accent/40 bg-accent/10 px-4 py-3 mb-4 text-sm text-ink">
          <span className="font-medium text-accent">Failed to reach BFF.</span>{" "}
      ...
      <div className="border-section pt-4">
        <StatusRow
          name="Fubon Neo SDK"
          desc="DMA login · trades / books / ticks / snapshot / tickers channels"
          status={data?.fubon_status}
          lastError={data?.fubon_last_error}
        />
        <StatusRow
          name="Supabase"
          desc="Postgres — symbols, strategies, signals_log"
      ...
      <p className="mt-6 text-xs text-ink-dim italic font-serif">
        Refreshed every five seconds. Backend at{" "}
...
function QuoteLookup() {
  ...
  return (
    <section>
      ...
      <h2 className="h-display text-[36px] mb-6">Quote a symbol</h2>
      ...
      <form onSubmit={onSubmit} className="bg-bg-card border border-line p-8 max-md:p-6">
        <div className="flex items-center border-b-2 border-line-strong pb-2 mb-6">
          <span className="text-2xs uppercase tracking-[2px] text-ink-dim mr-4">Symbol</span>
          <input
            type="text"
            value={pending}
        ...
        {error && (
          <div className="border border-accent/40 bg-accent/10 px-4 py-3 text-sm">
            <div className="font-medium text-accent mb-1">Quote failed</div>
      ...
      <p className="mt-6 text-xs text-ink-dim italic font-serif">
        Quote refreshed manually. Phase 3 will wire this to the WebSocket pool for live ticks.
      </p>
    </section>
  );
}
  ...
  return (
    <>
      ...
      <div className="text-xs text-ink-dim tracking-[1.5px] mb-6">
        ...
        {q.isClose && <span className="ml-3 text-accent">[ CLOSED ]</span>}
      </div>

      <div className="flex items-baseline gap-4 border-y border-line py-4 mb-6">
        <span className="label-tiny">Last close</span>
      ...
      <div className="grid grid-cols-3 max-md:grid-cols-2 gap-y-4 gap-x-8 border-b border-line pb-6">
        <Stat label="Open" value={fmtN(q.openPrice)} />
        <Stat label="High" value={fmtN(q.highPrice)} className="text-bull" />
        <Stat label="Low" value={fmtN(q.lowPrice)} className="text-bear" />
        <Stat label="Volume" value={fmtN(q.total?.tradeVolume)} />
        <Stat
          label="Turnover"
          value={
            q.total?.tradeValue
        ...
        />
        <Stat label="Amplitude" value={fmtPct(q.amplitude)} />
      </div>
