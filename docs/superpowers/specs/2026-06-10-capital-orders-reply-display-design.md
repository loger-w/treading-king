...
- **活單**才出現 `[刪單]` `[改]`(後端按 `_RANK` 算好 `actionable` 下發 = 預約中/委託成功/改價/改量/改價改量/部分成交;前端不自己抄狀態表):刪單 → 確認彈窗(prod 紅底,顯示該單摘要)→ POST cancel;`[改]` → 列內展開小表單(價格、減量張數,預填現值)→ 確認彈窗 → POST。終結態(全部成交/已刪單/失敗/退單)無按鈕
...
- **聚合非冪等**:同一筆回報事件 apply 兩次會重複累計成交量。v1 安全(ConnectByID 僅啟動重播一次);未來加回報斷線重連時,重播前必須 `store.clear()`。
- **D 事件帶 OrderErr**:不採計量並標失敗;**D 無價**:不採計量、不動狀態(無樣本,保守處理 — 少算成交 → remaining 高估 → 改價金額閘更嚴)。首次實際遇到看 log 驗證語意。
- **動作失敗(C/U/P/B + OrderErr)**:失敗的是該次刪/改動作,原單仍掛在市場 — 只記 error_msg、不動單狀態(N/D/S + err 才標整張單失敗/逾時)。標終態會讓活單的刪/改鈕消失、user 跟丟真錢委託。無真實樣本,依此語意實作+測試;首遇看 log。
- **預約單 KeyNo(idx0)≠ 尾欄序號(idx47)**(2026-06-10 真實樣本;盤中單兩者相同)。刪/改/減現傳 KeyNo,群益 API 認哪個未定 — 首測挑預約單,若回「查無委託」,看 log 警示行的尾欄值改試,把答案寫回本節。
- **寫入鏈市場閘**:已知非證券市場的 seq_no,三支寫入(刪/改價/減量)一律拒並留稽核 — 期貨口數會讓改價金額閘(價×量)低估數十倍名目曝險;期貨刪改連同期貨帳號/API 留待期貨面板。
