...
- **點價送單**:左欄=該價掛買、右欄=該價掛賣;固定**限價 ROD**(IOC/FOK 留在下單匣);張數/種類=面板當前值;無券時買側整排鎖住;同一格(價位×方向)500ms 防抖。
- **階梯生成與灰區判定抽 `lib/flash-ladder.ts` 純函式**(吃 現價/reference_price/掛單聚合 → 回列陣列),元件只負責渲染——對齊「無 hook 測試環境就抽 lib」慣例。
